...
const Header4 = ({ title }) => {
  DocumentTitle("Facebook video to MP4  | Fast & Free Online Converter")
  ...
  const handleClick = async (e) => {
    ...
    const options = {
      ...
      params: {
        url: `${url}`,
        filename: `facebook-${generateRandomString(10)}-(dowloaded from vidsaver.onrender.com)`
      },
      headers: {
    ...
    try {
      ...
      toast.update(loading, { render: "✅ Successfully gotten data", theme: theme, type: "success", icon: false, isLoading: false, autoClose: 2000, closeButton: true, hideProgressBar: false })
      ...
      toast.update(loading, { render: `💥  ${error.message}`, theme: theme, type: "error", icon: false, isLoading: false, autoClose: 5000, closeButton: true, hideProgressBar: false })
      console.error(error);
    }
  };
