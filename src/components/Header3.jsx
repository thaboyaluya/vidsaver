...
const Header3 = ({ title }) => {
  DocumentTitle("Tiktok video to MP4   | Fast & Free Online Converter")
  ...
  const handleClick = async (e) => {
    ...
    const options = {
      ...
      params: {
        url: `${url}`,
        filename: `tiktok-${generateRandomString(10)}-(dowloaded from vidsaver.onrender.com)`
      },
      headers: {
          ...
          <div className="col-lg-9 mx-auto">
            ...
            <div className="input-group w-100 mb-3">
              <input required onChange={(e) => setUrl(e.target.value)}
                value={url} className="form-control lb h-auto" type="text" placeholder="Paste link here..." />
            ...
            <div id='info' style={{ display: 'none' }}>
              ...
              <h3><i className="fa fa-share pr-1" aria-hidden="true"></i> Shares  : {video.stats ? video.stats.shareCount : 'no video views'} </h3>
              <h3><i className="fa fa-comments pr-1" aria-hidden="true"></i> Comments  : {video.stats ? video.stats.commentCount : 'no video comments conut'} </h3>
              <h3><i className="fa fa-play pr-1" aria-hidden="true"></i> Plays  : {video.stats ? video.stats.playCount : 'no video plays'} </h3>
              <h3><i className="fa fa-clock-o pr-1" aria-hidden="true"></i> Response time: {video.timeTaken ? video.timeTaken : 'no video timeTaken'} </h3>

              <h3>
                <i className="fa fa-music pr-2" aria-hidden="true"></i>

                <Link to={video.stats ? video.stats.music.playUrl : 'no sound title'} download={video.stats ? video.stats.music.playUrl : 'no sound title'}>

                  {video.stats ? video.stats.music.title : 'no video music title'}
